...
def generate_rst_files(app: Sphinx):
    ...
    with open(doc_path / "index", "w") as index_file:
    ...
    for module_info in pkgutil.walk_packages([package_toindex], prefix=""):
        ...
        module_rst_path = os.path.join(doc_path, f"{full_module_name}.rst")
    ...
    index_path = os.path.join(app.srcdir, "api.rst")
    with open(index_path, "w", encoding="utf-8") as f:
        f.write("API\n===\n\n")
        f.write(".. toctree::\n   :maxdepth: 2\n\n")
        for module_info in pkgutil.walk_packages([package_toindex]):
            if not module_info.ispkg:
                f.write(f"   api/{module_info.name}\n")